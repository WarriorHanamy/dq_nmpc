...
def generate_launch_description():
    ...
    launch_args.append(DeclareLaunchArgument("model_path", default_value=model_path))

    # Path config file for ROS layer (dq_control.yaml)
    ...
    # Path config file for NMPC solver (nmpc.yaml)
    nmpc_config = PathJoinSubstitution(
        [
            get_package_share_directory("dq_nmpc"),
            TextSubstitution(text="config"),
            platform_type,
            TextSubstitution(text="default"),
            TextSubstitution(text="nmpc.yaml"),
        ]
    )
    ...
    planner_node = Node(
        ...
        namespace=name,
        output="screen",
        parameters=[control_config, {"world_frame_id": world_frame_id, "body_frame_id": name}],
    )
    ...
    ld.add_action(
        GroupAction(actions=[quadrotor_simulator_mujoco_node, nmpc_controller_node, planner_node])
    )
    return ld
