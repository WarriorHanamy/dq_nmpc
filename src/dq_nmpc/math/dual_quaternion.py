...
class DualQuaternion:
    ...
    )  # Real part of the DualQuaternion, not initialized by default.
    Qd: Quaternion = field(
        init=False, default=None
    ...
    def __init__(self, q_real: Quaternion | None = None, q_dual: Quaternion | None = None) -> None:
        ...
        if q_real is not None and q_dual is not None:
            ...
            if not all(isinstance(i, Quaternion) for i in [q_real, q_dual]):
                raise ValueError(
                    "Elements of the DualQuaternions must be instances of the Quaternion class."
                )
            # Ensure that the numerical types of the quaternions' values match among np.ndarray, cs.MX, or cs.SX.
            elif (
                (isinstance(q_real.get, np.ndarray) and isinstance(q_dual.get, np.ndarray))
                or (isinstance(q_real.get, cs.MX) and isinstance(q_dual.get, cs.MX))
                or (isinstance(q_real.get, cs.SX) and isinstance(q_dual.get, cs.SX))
            ):
                ...
                raise ValueError(
                    "Elements of the DualQuaternions should be Quaternions of the same type: np.array, cs.MX, or cs.SX."
                )
        else:
    ...
    def __rmul__(self, q2: DualQuaternion | Number) -> DualQuaternion:
        ...
        if isinstance(q2, Number):
            ...
            if q2.shape[0] == 1:
                ...
                raise TypeError(
                    "Right Multiplication with DualQuaternion is only defined for scalars of the same type."
                )
    ...
    def product(p: DualQuaternion, q: DualQuaternion) -> DualQuaternion:
        ...
        if isinstance(q1r.get, np.ndarray) and isinstance(
            q2r.get, np.ndarray
        ):  # Use Vector directly without parentheses
            real = q1r * q2r
    ...
    def get(self) -> Vector:
        ...
        if isinstance(Qr_aux, np.ndarray) and (
            Qd_aux,
            np.array,
        ):  # Use Vector directly without parentheses
    ...
    def norm_dual_control(self) -> tuple[Scalar, Scalar]:
        ...
        if isinstance(qr.get, np.ndarray) and isinstance(
            qd.get, np.ndarray
        ):  # Use Vector directly without parentheses
            real_norm = qr.norm
    ...
    def set(self, q_real: Quaternion | None = None, q_dual: Quaternion | None = None) -> None:
        ...
        if q_real is not None and q_dual is not None:
            if not all(isinstance(i, Quaternion) for i in [q_real, q_dual]):
                ...
                raise ValueError(
                    "Elements of the DualQuaternions should be Quaternions of the same type: np.array. cs.MX or c.SX"
                )
        else:
    ...
    def axis_angle_dual(self) -> DualQuaternion:
        ...
        if isinstance(q1r.get, np.ndarray) and isinstance(
            ...
            Dual_ln = DualQuaternion(
                q_real=Quaternion(q=q1r_angle_axis), q_dual=Quaternion(q=result)
            )
    ...
    def H_plus_dual(self) -> Vector:
        ...
        if isinstance(q1r.get, np.ndarray) and isinstance(
            ...
            zeros = np.zeros((4, 4))
            Hplus = np.block([[H_r_plus, zeros], [H_d_plus, H_r_plus]])
            # Get translation of the dual quaternion
        elif isinstance(q1r.get, cs.MX) and isinstance(q1d.get, cs.MX):
            # Get translation of the dual quaternion
            zeros = cs.MX.zeros(4, 4)
            Hplus = cs.vertcat(cs.horzcat(H_r_plus, zeros), cs.horzcat(H_d_plus, H_r_plus))
        elif isinstance(q1r.get, cs.SX) and isinstance(q1d.get, cs.SX):
            # Get translation of the dual quaternion
    ...
    def H_minus_dual(self) -> Vector:
        ...
        if isinstance(q1r.get, np.ndarray) and isinstance(
            ...
            zeros = np.zeros((4, 4))
            Hminus = np.block([[H_r_minus, zeros], [H_d_minus, H_r_minus]])
            # Get translation of the dual quaternion
        elif isinstance(q1r.get, cs.MX) and isinstance(q1d.get, cs.MX):
            # Get translation of the dual quaternion
            zeros = cs.MX.zeros(4, 4)
            Hminus = cs.vertcat(cs.horzcat(H_r_minus, zeros), cs.horzcat(H_d_minus, H_r_minus))
        elif isinstance(q1r.get, cs.SX) and isinstance(q1d.get, cs.SX):
            # Get translation of the dual quaternion
            zeros = cs.SX.zeros(4, 4)
            Hminus = cs.vertcat(cs.horzcat(H_r_minus, zeros), cs.horzcat(H_d_minus, H_r_minus))
