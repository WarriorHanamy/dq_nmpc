...
def solver(params, flag=True):
    ...
    ocp.model.cost_expr_ext_cost = (
        ln_error.T @ Q_dual @ ln_error
        + error_u.T @ R_u @ error_u
        + error_v.T @ Q_v @ error_v
        + error_w.T @ Q_w @ error_w
    )
    ocp.model.cost_expr_ext_cost_e = (
        ln_error.T @ Q_dual @ ln_error + error_v.T @ Q_v @ error_v + error_w.T @ Q_w @ error_w
    )
    ...
    ocp.solver_options.qp_solver_cond_N = params["nmpc"][
        "horizon_steps"
    ]  # Number of QP stages for partial condensing
